...
#Created 2026-04-22 at 10:31

xpose.runno <- '1'
toolname <- 'execute'
pdf.filename <- paste0('PsN_',toolname,'_plots.pdf')
working.directory<-'/Users/walker/Desktop/Demo/run1/'
results.directory <- '..'
subset.variable<-NULL
tab.suffix <- '' 
rscripts.directory <- '/opt/homebrew/Cellar/perl/5.42.2/lib/perl5/site_perl/5.42/PsN_5_7_0/R-scripts' # This is not used
raw.results.file <- 'raw_results_run1.csv'
theta.labels <- c('[CL]','[V]','[KA]')
theta.fixed <- c(FALSE,FALSE,TRUE)
omega.labels <- c('[CL] omega(1,1)','[V] omega(2,2)')
omega.fixed <- c(FALSE,FALSE)
sigma.labels <- c('[A] sigma(2,2)','[A] sigma(2,2)_')
